...
if(life_cash < _basePrice) then {
	if(_debitcard == 0) exitwith {_succes = false;};
	if(life_cash < _debitcard) then {_debitcard = life_atmcash};
	if(_debitcard < _basePrice) exitwith {_succes = false;};
	life_atmcash = life_atmcash - _basePrice;
} else {
	life_cash = life_cash - _basePrice;
};
if (!_succes) exitwith {hint format[localize "STR_Shop_Veh_NotEnough",[_basePrice - life_cash] call life_fnc_numberText];};
...
[_vehicle,"vehicle_info_owners",[[getPlayerUID player,profileName]],true] remoteExec ["TON_fnc_setObjVar",2];
	...
	_vehicle setDir (markerDir _spawnPoint);
	_vehicle setPos (getMarkerPos _spawnPoint);
	_vehicle setVariable ["A3L_GunRack",[[],[]],true];
[_vehicle,_colorIndex] remoteExec ["life_fnc_colorVehicle",0];
	[_vehicle] call life_fnc_clearVehicleAmmo;
[_vehicle,"trunk_in_use",false,true] remoteExec ["TON_fnc_setObjVar",2];
[_vehicle,"vehicle_info_owners",[[getPlayerUID player,profileName]],true] remoteExec ["TON_fnc_setObjVar",2];
...
if(_className == "I_MRAP_03_F") then {
	if(_colorIndex in [0,1]) then {
		[_vehicle,"mrap_police",true] spawn life_fnc_vehicleAnimate;
	};
};		

...
if ((player distance bank) < 10) then {
	...
	if (copamount < 5) exitwith {["There need to be at least 5 cops on Duty!",30,"red"] call A3L_Fnc_Msg;};
...
if ((player distance prisondoor) < 10) then {
	...
	private ["_copamount"];
	if ((side player) == west) exitwith {prisondoor animate ['maindoor1', 0]; prisondoor animate ['maindoor2', 0]; ["Prison Entrance closed.",30,"red"] call A3L_Fnc_Msg; };
	_copamount = 0; 
	{if (side _x == west) then {_copamount = _copamount + 1};} foreach playableUnits;
	if (_copamount < 5) exitwith {["There need to be at least 5 cops on Duty!",30,"red"] call A3L_Fnc_Msg;};

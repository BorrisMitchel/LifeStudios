...
if((lbCurSel 5932) == -1) exitWith {hint localize "STR_ISTR_SelectItemFirst";};
...
switch (true) do
{
	case (_item == "earplugs"):
	{
            if (life_earplugsActive) exitWith {hint "You are already using earplugs.";};
            if (([false,_item,1] call life_fnc_handleInv)) then {
                    [] spawn {
                            if(!life_earplugsActive) then {
                                    1 fadeSound 0.2;
                                    titleText ["You have put in your earplugs.", "PLAIN"];
                                    life_earplugsActive = true;
                                    earplugs = player addAction ["<t color='#FF0000'>Remove Earplugs</t>", {
 
                                        1 fadeSound 1.0;
                                        titleText ["Your earplugs have been removed.", "PLAIN"];
                                        life_earplugsActive = false;
                                        player removeAction earplugs;
	...
	case (_item == "grantgunlicence"):
	{
		if(([false,_item,1] call life_fnc_handleInv)) then
		{
			license_civ_rifle = true;
		};
	};
	...
	case (_item == "repairkit"):
	{
		closedialog 0;[] spawn life_fnc_repairkit;
	};
	
	case (_item == "batterypack"):
	{
	if(([false,_item,1] call life_fnc_handleInv)) then
		{
		batterylevel = batterylevel + 50;
		if (batterylevel > 100) then { batterylevel = 100; };
		["You succesfully fast-charged your phone!",20,"green"] call A3L_Fnc_Msg;
		};
	};

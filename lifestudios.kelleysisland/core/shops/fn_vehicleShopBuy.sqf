...
switch(playerSide) do {
	case west: {
		...
		if(_className == "AM_09Tahoe_P_P") then {
			[_vehicle,"am_car",true] spawn life_fnc_vehicleAnimate;
		};
		if(_className == "AM_09Tahoe_U_T") then {
			[_vehicle,"am_car",true] spawn life_fnc_vehicleAnimate;
		};
		if(_className == "AM_10Fusion_P_P") then {
			[_vehicle,"am_car",true] spawn life_fnc_vehicleAnimate;
		};
	};
	...
	case independent: {
		...
		if(_className == "IVORY_PRIUS") then {
			[_vehicle,"sports2_prius",true] spawn life_fnc_vehicleAnimate;
		};
		if(_className == "AM_09Tahoe_E_P") then {
			[_vehicle,"am_car",true] spawn life_fnc_vehicleAnimate;
		};
	};
};

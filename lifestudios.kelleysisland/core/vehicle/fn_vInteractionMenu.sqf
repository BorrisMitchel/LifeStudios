...
#define Btn9 37458
#define Title 37401
private["_display","_curTarget","_Btn1","_Btn2","_Btn3","_Btn4","_Btn5","_Btn6","_Btn7","_Btn8","_Btn9","_package"];
if(!dialog) then {
...
_Btn8 ctrlSetText "Push Car";
_Btn8 buttonSetAction "closeDialog 0; [life_vInact_curTarget] spawn life_fnc_pushVehicle2;";
...
if(cursorTarget in life_vehicles) then {_Btn8 ctrlEnable true;} else {_Btn8 ctrlEnable false;};
if(locked cursortarget == 1) then {_Btn8 ctrlEnable true;} else {_Btn8 ctrlEnable false;};
...
if(playerSide == west) then {
	...
	if(_curTarget isKindOf "Ship") then {
		...
		if(typeOf (_curTarget) in ["C_Kart_01_Blu_F","C_Kart_01_Red_F","C_Kart_01_Fuel_F","C_Kart_01_Vrana_F"]) then {
			...
			// If the vehicle is one of the A3L bike's allow the unflip even if there if crew returns 0
			if ((typeof _curTarget) IN ["A3L_Bicycle","A3L_OldBike","M1030"]) then {_Btn7 ctrlEnable true;};
		};
	};
